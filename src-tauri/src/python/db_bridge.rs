//! 数据库桥接模块
//!
//! 供 Python 通过 PyO3 调用 Rust 的数据库方法
    ...
    Python::with_gil(|py| {
        ...
        let save_video_fn = pyo3::types::PyCFunction::new_closure_bound(
            ...
            move |args: &Bound<'_, pyo3::types::PyTuple>, _kwargs: Option<&Bound<'_, pyo3::types::PyDict>>| -> PyResult<()> {
                ...
                // 将 Python dict 转为 JSON，再反序列化为 VideoInfo
                let py = data.py();
                let json = py.import_bound("json")?;
                let json_str: String = json.call_method1("dumps", (data,))?.extract()?;
                let video_info: crate::db::VideoInfo = serde_json::from_str(&json_str)
        ...
        let save_user_fn = pyo3::types::PyCFunction::new_closure_bound(
            ...
            move |args: &Bound<'_, pyo3::types::PyTuple>, _kwargs: Option<&Bound<'_, pyo3::types::PyDict>>| -> PyResult<()> {
                ...
                let json = py.import_bound("json")?;
                let json_str: String = json.call_method1("dumps", (data,))?.extract()?;
                let user_info: crate::db::UserInfo = serde_json::from_str(&json_str)
